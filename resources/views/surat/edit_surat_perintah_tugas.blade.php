...
@section('content')
    @php
        ...
        if (!is_array($editDasar)) {
            if (is_array($surat->dasar ?? null)) {
                $editDasar = array_values($surat->dasar);
            } elseif (filled($surat->dasar ?? null)) {
                $editDasar = [(string) $surat->dasar];
            } else {
                $editDasar = [''];
            }
        }

        if (count($editDasar) === 0) {
            $editDasar = [''];
        }
         ...
         * =========================================================
         */
        $editPenerima = old('penerima_tugas');

        if (!is_array($editPenerima)) {
            if (is_array($surat->penerima_tugas ?? null)) {
                $editPenerima = array_values($surat->penerima_tugas);
            } elseif (
                filled($surat->nama_penerima ?? null) ||
                filled($surat->jabatan_penerima ?? null)
            ) {
                // Fallback untuk data lama.
                $editPenerima = [
                    [
                        'nama' => $surat->nama_penerima ?? '',
                        'kedudukan' => $surat->jabatan_penerima ?? '',
                    ],
                ];
            } else {
                $editPenerima = [
        ...
        if ($editUntuk === null) {
            $editUntuk = $surat->untuk ?? '';

            /*
             * Fallback data lama agar isi tugas lama tetap terlihat.
             */
            if (blank($editUntuk)) {
                $bagianUntuk = [];

                if (filled($surat->untuk_mengikuti ?? null)) {
                    $bagianUntuk[] = trim(
                        (string) $surat->untuk_mengikuti
                    );
                }

                $detailKegiatan = [];

                if (filled($surat->hari ?? null)) {
                    $detailKegiatan[] =
                        'pada hari ' . trim((string) $surat->hari);
                }

                if (filled($surat->tanggal_kegiatan ?? null)) {
                    try {
                ...
                if (filled($surat->waktu_mulai ?? null)) {
                    $detailKegiatan[] =
                        'pukul ' .
                        trim((string) $surat->waktu_mulai);
                }

                if (filled($surat->tempat_kegiatan ?? null)) {
                    $detailKegiatan[] =
                        'bertempat di ' .
                        trim((string) $surat->tempat_kegiatan);
                }

                if (count($detailKegiatan) > 0) {
                    $bagianUntuk[] =
                        implode(', ', $detailKegiatan);
                }

                if (filled($surat->keterangan_tugas ?? null)) {
                    $bagianUntuk[] =
                        trim((string) $surat->keterangan_tugas);
                }

                $editUntuk = implode(
                    '. ',
                    array_filter($bagianUntuk)
                );

                if (
                    filled($editUntuk) &&
                    !str_ends_with($editUntuk, '.')
                ) {
                    $editUntuk .= '.';
                }
            }
            ...
            <div class="col-12">
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <form
                            action="{{ route('surat.perintah_tugas.update', $surat->_id) }}"
                            method="POST"
                            id="formEditSuratPerintahTugas"
                        >
                            @csrf
                            @method('PUT')
                            ...
                            <div id="dasarContainer">
                                @foreach ($editDasar as $index => $dasar)
                                    <div
                            ...
                            <div id="penerimaContainer">
                                @foreach ($editPenerima as $index => $penerima)
                                    <div
                            ...
                            <div class="d-flex flex-wrap justify-content-end gap-2 mt-4">
                                ...
                                <button
                                    ...
                                    id="submitEditSuratPerintahTugas"
                                >
                                    Update Surat
                                </button>
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const form =
                document.getElementById('formEditSuratPerintahTugas');

            const submitButton =
                document.getElementById('submitEditSuratPerintahTugas');
