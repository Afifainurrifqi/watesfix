                ...
                <form action="{{ route('surat.skck.update', $surat->_id ?? $surat->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nik" class="form-control" required
                            value="{{ old('nik', $surat->nik) }}" maxlength="16" inputmode="numeric">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jenis_kelamin" class="form-control" required>
                            ...
                            @foreach (['Laki-laki', 'Perempuan'] as $opt)
                                <option value="{{ $opt }}" {{ $jk == $opt ? 'selected' : '' }}>{{ $opt }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="agama" class="form-control" required>
                            ...
                            @foreach (['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu', 'Lainnya'] as $opt)
                                <option value="{{ $opt }}" {{ $ag == $opt ? 'selected' : '' }}>
                                    {{ $opt }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        @php $pd = old('pendidikan', $surat->pendidikan); @endphp

                        <select name="pendidikan" class="form-control" required>
                            <option value="">-- Pilih Pendidikan --</option>

                            @foreach (['TIDAK/BLM SEKOLAH', 'BELUM TAMAT SD/SEDERAJAT', 'TAMAT SD/SEDERAJAT', 'SLTP/SEDERAJAT', 'SLTA/SEDERAJAT', 'DIPLOMA I/II', 'AKADEMI/DIPLOMA III/SARJANA MUDA', 'DIPLOMA IV/STRATA I', 'STRATA-II', 'STRATA-III'] as $opt)
                                <option value="{{ $opt }}" {{ $pd == $opt ? 'selected' : '' }}>
                                    {{ $opt }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="pekerjaan" id="pekerjaan" class="form-control" required>
                            ...
                            @foreach ($jobs as $job)
                                <option value="{{ $job }}" {{ $pk == $job ? 'selected' : '' }}>
                                    {{ $job }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="keperuntukan" class="form-control" required
                            value="{{ old('keperuntukan', $surat->keperuntukan) }}">
                    </div>
                    ...
                    <div class="text-end">
                        ...
                        <a href="{{ route('surat.skck.index') }}" class="btn btn-danger">Batal</a>
                    </div>
                </form>
            </div>
        </div>
