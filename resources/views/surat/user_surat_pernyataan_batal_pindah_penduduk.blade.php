                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status" id="status" class="form-control" required>
                                    ...
                                    @foreach($status ?? [] as $item)
                                        <option value="{{ $item->nama }}" {{ old('status') == $item->nama ? 'selected' : '' }}>
                                            {{ $item->nama }}
                                        </option>
                                    @endforeach
                                </select>
            ...
            fetch(`/datapenduduk/lookup/${nik}`)
                ...
                .then(result => {
                    if (result.success && result.data) {
                        ...
                        // ==================== AUTOFILL DROPDOWN AGAMA & STATUS ====================
                        if (document.getElementById('agama') && d.agama) {
                            document.getElementById('agama').value = d.agama;
                        }
                        if (document.getElementById('status') && (d.status_perkawinan || d.status)) {
                            document.getElementById('status').value = d.status_perkawinan || d.status;
                        }
                        // =====================================================================
                    }
                });
