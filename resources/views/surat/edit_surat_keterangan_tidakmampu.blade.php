{{-- resources/views/surat/tidakmampu/edit.blade.php --}}
...
@section('content')
    <div class="container">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <h4 class="mb-4">Edit Surat Keterangan Tidak Mampu</h4>

                <form action="{{ route('surat.tidakmampu.update', $sktm->id) }}" method="POST" novalidate>
                    @csrf
                    @method('PUT')
                    ...
                    @php
                        ...
                        // Map bantuan -> label (key stabil untuk name input)
                        $bansosMap = [
                            'pkh' => 'PKH',
                            'kip' => 'KIP',
                            'kis' => 'KIS',
                        ...
                        // Ambil value existing dari model (asumsikan kolom JSON/array)
                        $existingBantuan = is_array($sktm->bantuan ?? null)
                            ? $sktm->bantuan
                            : (empty($sktm->bantuan)
                                ? []
                                : (array) $sktm->bantuan);
                        $existingBantuanId = is_array($sktm->bantuan_id ?? null)
                            ? $sktm->bantuan_id
                            : (empty($sktm->bantuan_id)
                                ? []
                                : (array) $sktm->bantuan_id);

                        // Old() fallback ke existing
                        $oldBantuan = old('bantuan', $existingBantuan);
                        $oldIds = old('bantuan_id', $existingBantuanId);

                        // Tanggal lahir (aman untuk string/timestamp/null)
                        $tglLahir = old(
                            'tanggal_lahir',
                            !empty($sktm->tanggal_lahir) ? Carbon::parse($sktm->tanggal_lahir)->format('Y-m-d') : null,
                        );
                        ...
                        $pekerjaanNow = old('pekerjaan', $sktm->pekerjaan ?? '');

                        $normalisasi = function ($value) {
                            return strtoupper(trim((string) $value));
                        };

                        $pekerjaanAdaDiList = collect($jobs)->contains(function ($job) use (
                            $pekerjaanNow,
                            $normalisasi,
                        ) {
                            return $normalisasi($job) === $normalisasi($pekerjaanNow);
                        });

                        if (!empty($pekerjaanNow) && !$pekerjaanAdaDiList) {
                            array_unshift($jobs, $pekerjaanNow);
                        }
                    @endphp
                        ...
                        <select name="peruntukan_sktm" id="peruntukan_sktm"
                            ...
                            @foreach (['Biaya Pendidikan', 'Bantuan Sosial', 'Biaya Kesehatan'] as $peruntukan)
                                <option value="{{ $peruntukan }}"
                                    {{ old('peruntukan_sktm', $sktm->peruntukan_sktm) == $peruntukan ? 'selected' : '' }}>
                                    {{ $peruntukan }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_lengkap" id="nama_lengkap"
                            ...
                            value="{{ old('nama_lengkap', $sktm->nama_lengkap) }}">
                        @error('nama_lengkap')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="kewarganegaraan" id="kewarganegaraan"
                            ...
                            value="{{ old('kewarganegaraan', $sktm->kewarganegaraan) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="agama" id="agama" class="form-control @error('agama') is-invalid @enderror"
                            ...
                            @foreach (['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Khonghucu'] as $agama)
                                <option value="{{ $agama }}"
                                    {{ old('agama', $sktm->agama) == $agama ? 'selected' : '' }}>
                                    {{ $agama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_perkawinan" id="status_perkawinan"
                            ...
                            @foreach (['Kawin', 'Belum Kawin', 'Cerai Hidup', 'Cerai'] as $status)
                                <option value="{{ $status }}"
                                    {{ old('status_perkawinan', $sktm->status_perkawinan) == $status ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="pekerjaan" id="pekerjaan"
                            ...
                            @foreach ($jobs as $job)
                                <option value="{{ $job }}"
                                    {{ $normalisasi($pekerjaanNow) === $normalisasi($job) ? 'selected' : '' }}>
                                    {{ $job }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="alamat_rumah" id="alamat_rumah" rows="3"
                            class="form-control @error('alamat_rumah') is-invalid @enderror" required>{{ old('alamat_rumah', $sktm->alamat_rumah) }}</textarea>
                        @error('alamat_rumah')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="keterangan_fungsi_surat" class="form-label">Keterangan Fungsi Surat (Kelengkapan)
                            <span class="text-danger">*</span></label>
                        <textarea name="keterangan_fungsi_surat" id="keterangan_fungsi_surat" rows="3"
                            class="form-control @error('keterangan_fungsi_surat') is-invalid @enderror" required>{{ old('keterangan_fungsi_surat', $sktm->keterangan_fungsi_surat) }}</textarea>
                        ...
                        <select name="status_surat" id="status_surat"
                            ...
                            @foreach (['Pending', 'Di cek', 'Di terima', 'Ditolak'] as $statusSurat)
                                <option value="{{ $statusSurat }}"
                                    {{ old('status_surat', $sktm->status_surat) == $statusSurat ? 'selected' : '' }}>
                                    {{ $statusSurat }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="status_verif" id="status_verif"
                            ...
                            @foreach (['Belum Verifikasi', 'Terverifikasi'] as $statusVerif)
                                <option value="{{ $statusVerif }}"
                                    {{ old('status_verif', $sktm->status_verif) == $statusVerif ? 'selected' : '' }}>
                                    {{ $statusVerif }}
                                </option>
                            @endforeach
                        </select>
    ...
    {{-- Inline Script agar toggle langsung jalan --}}
    <script>
        ...
        // Inisialisasi saat halaman dibuka (cover old() dan existing value)
